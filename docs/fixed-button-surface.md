...
| --- | --- | --- | --- | --- | --- | --- |
| Prepare Article From Source Or Brief | Editor Content Support | public URL, with optional brief fields behind disclosure | optional `source_extraction_preview.v1`, then `article_writing_pack.v1`, request-scoped `article_writing_pack_review.v1`, and optional `article_draft_preview.v1` | Cloud exact-URL reader when needed, Cloud Site Knowledge vectors, and hosted suggestion/draft runtime | metadata/navigation-only extraction stops; otherwise the human confirms direction and may load a usable draft only into an empty current Gutenberg body | `default_button_v1`; no full translation, existing-body replacement, title/excerpt overwrite, local URL fetch, media import, approval store, Core proposal, automatic save, or publish |
