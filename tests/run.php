...
toolbox_assert( false !== strpos( $editor_recommendation_logic_doc, 'Users without that' ) && false !== strpos( $editor_recommendation_logic_doc, 'taxonomy capability should see existing tag recommendations only' ) && false !== strpos( $editor_recommendation_logic_doc, 'not by a hard-coded role name' ) && false !== strpos( $editor_recommendation_logic_doc, 'Toolbox still does not' ) && false !== strpos( $editor_recommendation_logic_doc, 'create terms from this panel' ), 'Editor recommendation logic documents capability-gated new tag gap visibility.' );
...
toolbox_assert( false !== strpos( $editor_support, "'capabilities'" ) && false !== strpos( $editor_support, "'canCreateTags'" ) && false !== strpos( $editor_support, "current_user_can_taxonomy_capability( 'post_tag', 'edit_terms' )" ), 'Post editor content support localizes post-tag creation capability for UI gating.' );
...
toolbox_assert( false !== strpos( $editor_support_json, '"Recommended existing tags": ["推荐已有标签"]' ) && false !== strpos( $editor_support_json, '"Proposed new tags for review only": ["拟新增标签（仅审查）"]' ) && false !== strpos( $editor_support_json, '"Review-only vocabulary gaps. Toolbox will not create or assign these tags from this panel.": ["仅用于审查的词汇缺口；Toolbox 不会从此面板创建或分配这些标签。"]' ) && false !== strpos( $editor_support_json, '"No existing tag choices are available for Core review.": ["当前没有可提交到 Core 审核的已有标签。"]' ), 'Bundled zh_CN editor script translation JSON distinguishes existing tags from proposed new tag gaps.' );
...
toolbox_assert( false !== strpos( $editor_support_json, '"Suggested handling list": ["建议处理列表"]' ) && false !== strpos( $editor_support_json, '"Open internal link candidates": ["打开内链候选"]' ) && false !== strpos( $editor_support_json, '"Review only. Toolbox does not insert links automatically.": ["仅供审阅。Toolbox 不会自动插入链接。"]' ), 'Bundled zh_CN editor script translation JSON covers preflight routing and review-only candidate policy copy.' );
...
foreach ( array( 'writing_support', 'title_suggestions', 'article_outline', 'polish_notes', 'publish_preflight', 'discoverability', 'summary_suggestions', 'category_suggestions', 'tag_suggestions', 'internal_links', 'image_candidates', 'image_alt_suggestions' ) as $editor_intent ) {
...
toolbox_assert( false !== strpos( $editor_js, 'Recommended existing tags' ) && false !== strpos( $editor_js, 'Proposed new tags for review only' ) && false !== strpos( $editor_js, 'Review-only vocabulary gaps. Toolbox will not create or assign these tags from this panel.' ) && false !== strpos( $editor_js, 'No existing tag choices are available for Core review.' ) && false !== strpos( $editor_js, 'if (canCreateTags)' ) && false !== strpos( $editor_js, 'canCreateTags && (newTermItems.length' ) && false === strpos( $editor_js, 'Only consider this as a new tag if no existing WordPress tag is close enough.' ), 'Editor Content Support hides review-only new tag gaps when the current user cannot create post tags.' );
...
toolbox_assert( false !== strpos( $editor_js, 'internalLinkCandidateItems' ) && false !== strpos( $editor_js, 'Internal link candidates' ) && false !== strpos( $editor_js, 'Review only, no insert' ), 'Editor Content Support renders internal-link candidates as review-only suggestions instead of raw Site Knowledge.' );
...
toolbox_assert( false !== strpos( $editor_js, 'renderInternalLinkCandidateSection' ) && false !== strpos( $editor_js, 'Toolbox does not insert links automatically.' ) && false === strpos( $editor_js, 'insertInternalLinkCandidate' ), 'Editor Content Support shows internal-link candidates as review-only cards without adding a default direct insertion path.' );
...
foreach ( array( 'writing_support', 'title_suggestions', 'article_outline', 'polish_notes', 'summary_suggestions', 'category_suggestions', 'tag_suggestions', 'summary_terms_optimization', 'taxonomy_tags', 'internal_links', 'image_candidates', 'image_alt_suggestions', 'publish_preflight', 'discoverability' ) as $editor_rest_intent ) {
...
toolbox_assert( false !== strpos( $client, "'timeout_seconds'     => 'summary_suggestions' === \$intent ? 60 : 30" ), 'Provider client gives full-draft summary generation a longer timeout than lightweight hosted draft-support actions.' );
