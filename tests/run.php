...
toolbox_assert( false !== strpos( $admin_page, 'Free GPT-5.5 Draft Support' ) && false !== strpos( $admin_page, 'free-gpt55-title-summary' ) && false !== strpos( $admin_page, 'free-gpt55-outline' ) && false !== strpos( $admin_page, 'free-gpt55-polish' ), 'Tool actions expose the free GPT-5.5 draft-support entry group.' );
toolbox_assert( false !== strpos( $admin_page, "'intent'      => 'title_summary'" ) && false !== strpos( $admin_page, "'intent'      => 'article_outline'" ) && false !== strpos( $admin_page, "'intent'      => 'polish_notes'" ), 'Free GPT-5.5 draft-support tools map to title, outline, and polish intents.' );
toolbox_assert( false === strpos( $admin_page, "'intent'      => 'site_checkup'" ) && false === strpos( $admin_page, "'intent'      => 'media_alt'" ) && false === strpos( $admin_page, "'intent'      => 'smart_recommendations'" ), 'Default GPT-5.5 draft-support tools do not expose site-level snapshot intents.' );
toolbox_assert( false !== strpos( $admin_page, 'Free GPT-5.5 hosted route' ) && false !== strpos( $admin_page, 'reviewable suggestion, not a finished article' ) && false !== strpos( $admin_page, "'powered_by'  => 'free_gpt55'" ), 'Free GPT-5.5 tools remain suggestion-only hosted-runtime entries.' );
...
toolbox_assert( false !== strpos( $admin_js, 'Title and summary suggestions' ) && false !== strpos( $admin_js, 'Outline suggestions' ) && false !== strpos( $admin_js, 'Polish suggestions' ), 'Admin JavaScript labels the free GPT-5.5 draft-support result modes.' );
toolbox_assert( false !== strpos( $admin_js, 'renderFreeGpt55QualityGuardrails' ) && false !== strpos( $admin_js, 'Review checklist' ) && false !== strpos( $admin_js, 'Expected output shape' ), 'Admin JavaScript renders GPT-5.5 quality guardrails without adding a heavier workflow.' );
...
toolbox_assert( false !== strpos( $editor_js, 'Media SEO preview' ) && false !== strpos( $editor_js, 'buildImageSeoFields' ) && false !== strpos( $editor_js, 'set_featured_image: true' ), 'Editor image adoption plan previews media SEO fields before Core handoff.' );
...
toolbox_assert( false !== strpos( $client, "'title_summary'" ) && false !== strpos( $client, "'article_outline'" ) && false !== strpos( $client, "'polish_notes'" ) && false !== strpos( $client, '? 650' ), 'Provider client keeps default GPT-5.5 draft-support intents lightweight.' );
toolbox_assert( false !== strpos( $client, 'free_gpt55_quality_contract' ) && false !== strpos( $client, "'quality_contract'" ) && false !== strpos( $client, "'review_checklist'" ) && false !== strpos( $client, "'reject_if'" ), 'Provider client attaches GPT-5.5 quality guardrails to prompts and normalized outputs.' );
...
toolbox_assert( false !== strpos( $client, 'free_gpt55_content_support_prompt' ) && false !== strpos( $client, 'direct_wordpress_write' ) && false !== strpos( $client, 'core_proposal_required' ) && false !== strpos( $client, 'Do not generate a full article' ), 'Free GPT-5.5 prompt preserves suggestion-only Core approval boundaries.' );
