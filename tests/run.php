...
toolbox_assert( false !== strpos( $admin_page, 'wp_enqueue_media()' ) && false !== strpos( $admin_page, 'adapterRestUrl' ) && false !== strpos( $admin_page, "rest_url( 'magick-ai-adapter/v1' )" ), 'Media Derivative panel loads WordPress media picker and Adapter REST config.' );
toolbox_assert( false !== strpos( $admin_page, 'Core remains the policy owner and final WordPress write owner' ), 'Media Derivative Handoff copy keeps Core as policy and final write owner.' );
toolbox_assert( false !== strpos( $admin_page, 'data-toolbox-media-derivative' ) && false !== strpos( $admin_page, 'data-toolbox-select-media' ) && false !== strpos( $admin_page, 'Generate preview' ), 'Media Derivative panel supports media library selection and Cloud preview generation.' );
toolbox_assert( false !== strpos( $admin_page, 'Submit replacement proposal' ) && false !== strpos( $admin_page, 'Submit URL repair proposal' ) && false !== strpos( $admin_page, 'Submit settings URL proposal' ), 'Media Derivative panel submits only Core-governed proposal requests for replacement and URL repairs.' );
toolbox_assert( false !== strpos( $admin_page, 'Batch conversion plan' ) && false !== strpos( $admin_page, 'data-toolbox-build-media-batch-plan' ) && false !== strpos( $admin_page, 'data-toolbox-run-media-batch-previews' ) && false !== strpos( $admin_page, 'data-toolbox-submit-media-batch-proposals' ), 'Media Derivative panel supports bounded batch candidate planning and selected proposal submission.' );
...
toolbox_assert( false !== strpos( $admin_js, 'initMediaDerivativeControls' ) && false !== strpos( $admin_js, 'window.wp.media' ), 'Admin JavaScript initializes WordPress media picker controls.' );
toolbox_assert( false !== strpos( $admin_js, 'createMediaDerivativePreview' ) && false !== strpos( $admin_js, 'media-derivative-runs' ) && false !== strpos( $admin_js, 'media-derivative-proposal-payload' ), 'Admin JavaScript generates media derivative previews through Adapter routes.' );
toolbox_assert( false !== strpos( $admin_js, "ability_id: 'magick-ai/adopt-cloud-media-derivative'" ) && false !== strpos( $admin_js, 'dry_run: true' ) && false !== strpos( $admin_js, 'commit: false' ), 'Admin JavaScript submits replacement work only as dry-run Core proposal payloads.' );
toolbox_assert( false !== strpos( $admin_js, 'buildMediaDerivativeBatchPlan' ) && false !== strpos( $admin_js, "ability_id: 'magick-ai/build-media-derivative-batch-plan'" ) && false !== strpos( $admin_js, 'runMediaDerivativeBatchPreviews' ) && false !== strpos( $admin_js, 'submitMediaDerivativeBatchProposals' ), 'Admin JavaScript supports bounded media derivative batch planning and proposal submission.' );
toolbox_assert( false !== strpos( $admin_js, "ability_id: 'magick-ai/build-media-reference-repair-plan'" ) && false !== strpos( $admin_js, "ability_id: 'magick-ai/build-media-settings-reference-repair-plan'" ) && false !== strpos( $admin_js, 'proposals/from-plan' ), 'Admin JavaScript routes hard-coded URL and settings repair through Core plan-to-proposal governance.' );
...
toolbox_assert( false !== strpos( $admin_css, 'magick-ai-toolbox__media-picker' ) && false !== strpos( $admin_css, 'magick-ai-toolbox__derivative-preview' ), 'Admin CSS supports local media selection and signed derivative previews.' );
toolbox_assert( false !== strpos( $admin_css, 'magick-ai-toolbox__batch-panel' ) && false !== strpos( $admin_css, 'magick-ai-toolbox__batch-row' ), 'Admin CSS supports batch media candidate review.' );
