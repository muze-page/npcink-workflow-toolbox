...
toolbox_editor_progressive_js_assert( false !== strpos( $editor_js, 'function shouldShowProgressiveRecommendationPanel' ) && false !== strpos( $editor_js, "status === 'warning' || status === 'error'" ) && false !== strpos( $editor_js, "status === 'success' && progressiveSuggestionCount(progressiveResult) === 0" ), 'Progressive recommendation panel only opens by request, failure, or empty-context success.' );
toolbox_editor_progressive_js_assert( false !== strpos( $editor_js, 'const [progressivePanelOpen, setProgressivePanelOpen] = useState(false)' ) && false !== strpos( $editor_js, 'function openProgressivePanel()' ) && false !== strpos( $editor_js, "__('Local suggestions', 'npcink-workflow-toolbox')" ), 'Progressive success state is represented by a compact Local suggestions entry instead of a default card.' );
